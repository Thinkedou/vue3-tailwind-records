...
// j'annonce les évenements que je vais pouvoir déclencher
const emit = defineEmits(['onIncreaseStock'])
...
const props = defineProps({
    ...
    coverUrl:{
        type:String,
        default:'src/assets/static/img/default.jpg'
    },
    year:{
        type:String
    },
...
const increaseStock = ()=>{
    // 📣 on déclenche vraiment l'événement 
 ...
 <section class="text-gray-600 body-font">
        <div class="container px-5 mx-auto ">
            <div class="p-5 bg-white flex items-center mx-auto border-b shadow-md mb-10 border-gray-400 rounded-lg sm:flex-row flex-col ">
                ...
                <h1 class="text-black text-2xl title-font font-bold mb-2">{{title }} {{ isFav?'⭐':'' }}</h1>
                <h3 class="text-black text-xl title-font mb-2">{{artist}}<span class="font-light ml-2">{{ year }}</span></h3>
                <p class="leading-relaxed text-base">{{ comment }}</p>
                <div class="py-4">
                
                    <div v-if="stock>0" class=" inline-block mr-2" > <!-- quand le stock est ok  -->
            ...
            <div class="w-full">
                <div class="float-right">
                    <button type="button" class="border border-teal-500 bg-teal-500 text-white rounded-md px-4 py-2 m-2 ease select-none hover:bg-teal-400" @click.stop="increaseStock"> + </button>
